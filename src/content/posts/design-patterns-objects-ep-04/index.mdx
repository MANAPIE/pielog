...
Object Adapter는 컴포지션(has-a), Class Adapter는 상속(is-a)이라는 차이입니다. 거의 모든 경우 컴포지션 쪽이 더 유연합니다. **이 시리즈가 강조하는 결정 — 객체의 책임을 어떻게 나눌 것인가 — 의 관점에서, 어긋난 인터페이스에 상속을 끼워 넣는 일은 책임을 두 곳으로 흩뜨립니다.**
...
Adapter가 답하는 질문 — 모양이 다른 두 세계를 한 객체로 잇는다 — 은 디자인 시스템 마이그레이션에서 정확히 같은 모양으로 다시 나타납니다.
