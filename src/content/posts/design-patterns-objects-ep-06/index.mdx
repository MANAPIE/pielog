...
**비즈니스 로직이 끼어든 Facade.** Facade는 협업을 정리하는 곳이지 도메인 로직을 결정하는 곳이 아닙니다. "VIP 사용자는 할인" 같은 정책이 Facade에 들어가면, 그 정책의 변경이 Facade와 클라이언트 양쪽에 파급됩니다. 정책은 별도의 객체에 두고 Facade는 호출만 합니다.
...
Facade가 답하는 질문 — 무엇을 표면으로 노출하고 무엇을 안쪽에 정리할 것인가 — 는 디자인 시스템 컴포넌트 라이브러리의 가장 중요한 결정입니다.
...
![디자인 시스템 컴포넌트 API의 표면적 결정을 보여주는 다이어그램. 가로축의 세 컬럼이 자유도에 따라 배치되어 있습니다. 좌측은 자유도 최소 — 단순한 variant prop만 받는 일반 Button 컴포넌트가 있고, 캡션에는 일관성은 높지만 예외 대응이 어렵다고 표시됩니다. 중앙은 균형 — 일반 Button 컴포넌트와 primitive 진입점이 동시에 제공되는 구조이며, 일반 사용은 Facade를 통하고 고급 사용은 primitive로 향하는 두 경로가 그려져 있습니다. 우측은 자유도 최대 — 모든 토큰을 prop으로 받는 비대한 컴포넌트로, 자유롭지만 일관성이 사라진다고 표시됩니다. 하단에는 좋은 디자인 시스템은 중앙의 균형 지점을 향한다는 화살표가 그려져 있습니다.](dpo-06-02-component-api-surface.svg)
