...
L2와 같은 `flowchart` 문법은 방향만 `LR`로 바꿔 *흐름이 옆으로 진행되는* 형태가 됐습니다. 외부 노드(`kafka`, `gateway`, `db`)를 `subgraph` 바깥에 두면 자동으로 *경계 바깥*에 그려집니다.
...
*변환 단계*가 명확한 시스템에 강합니다. ETL, 이벤트 파이프라인, 로그 처리 등이 대표적입니다. 양방향 통신이 많아지면 시스템 다이어그램으로 가야 합니다.
...
> **비용**
>
> - 메시지 *순서 보장*은 파티션 키 설계에 강하게 의존합니다. *가맹점 ID*를 키로 고정해야 합니다.
> - 운영 복잡도가 증가합니다. consumer lag·DLQ 모니터링이 추가되고, 분산 추적이 사실상 필수가 됩니다.
...
- **[Notion: Building and scaling Notion's data lake](https://www.notion.com/blog/building-and-scaling-notions-data-lake)** (XZ Tie 외, 2024-07-01).
  Snowflake에서 자체 data lake(S3 + Hudi + Spark)로 옮긴 이유. 블록 데이터가 6\~12개월마다 두 배로 늘어 *맥락*(비용·지연 폭증)이 명확하고, *대안*·*선택*·*결과*(비용·지연 개선의 정량적 그래프)·*비용*(운영 부담)을 다이어그램과 표로 풀어둡니다. *익스플레네이션이 측정치까지 같이 갈 수 있다*는 본보기입니다.
...
세 글의 공통점이 있습니다. *묘사*에서 끝나지 않고, 다이어그램은 있지만 *글의 중심*은 아니며, 자기 결정의 비용까지 솔직하게 적혀 있다는 것입니다. 그리고 모두 *결과의 측정치*가 본문 안에 있습니다. 이 셋만 갖춰도 익스플레네이션은 단순한 후기에서 *살아 있는 기록*으로 옮겨갑니다.
...
다음 편은 ADR과 RFC입니다. 같은 *의사결정*이지만 위치가 다른 두 문서입니다. ADR은 *이미 한 결정*을 박제하고, RFC는 *아직 안 한 결정*을 제안합니다. 그 차이가 글의 톤·구조·수명을 모두 다르게 만듭니다.
