...
**Logging and Alerting Failures(A09)** 는 OWASP Top 10에 오래 자리잡은 카테고리입니다. 2017년 "Insufficient Logging & Monitoring"으로 처음 등장해 2021년 "Security Logging and Monitoring Failures"로 이름이 다듬어졌고, 2025년판에서 "Alerting"이 명시적으로 추가됐습니다. 이름의 변화가 곧 강조점의 변화입니다. **로그를 쌓는 것보다 거기서 무엇이 발생했음을 알아채고 행동으로 연결하는 것이 중요**하다는 것입니다.
...
**Equifax (2017)**. 1억 4천 7백만 명의 개인정보가 유출됐습니다. 사후 조사에서 드러난 사실: 공격자가 **76일 동안 시스템 안에 머물면서** 9천 회 이상의 쿼리를 실행했습니다. 모니터링이 있었다면 그 정도 비정상 패턴을 그렇게 오래 놓치는 것은 어려웠을 것입니다. 게다가 조사 도중 **로그 분석 도구의 SSL 인증서가 만료**되어 있어 로그 자체가 일부 누락된 상태였습니다.

**Marriott / Starwood (2014~2018)**. 5억 명의 데이터가 유출됐습니다. 공격자가 **4년 동안** 시스템에 침투해 있었습니다. 이상 행동 탐지가 작동하지 않았거나 알림이 무시되고 있었습니다. 인수 전 Starwood 시스템부터 침투가 시작됐는데, 합병 후에도 발견되지 않았습니다.
...
**설정 변경**. 권한 부여, IAM 정책 수정, 시스템 환경 변경. 가장 자주 사고의 원인이 되는 영역입니다.

각 로그에는 공통 메타데이터가 들어갑니다. **타임스탬프(UTC, 밀리초)**, **사용자 ID**, **요청 ID(trace ID)**, **소스 IP**, **User-Agent**.

#### 절대 남기지 말아야 할 것

**자격증명**: 비밀번호(평문/해시 모두), API 키, JWT 전체, 세션 토큰, OAuth 토큰. JWT는 특히 주의해야 합니다. payload가 base64 인코딩만 되어 있어 로그에 그대로 새면 디코드 한 번이면 사용자 정보가 노출됩니다.
...
로그 시스템 자체가 공격 표적이 됩니다. 공격자가 가장 먼저 하는 일은 흔적 지우기입니다.
...
**Dwell Time (체류 시간)**: 공격자가 시스템 내에 머문 총 시간. 위 두 지표의 합. 2024년 Mandiant 보고서 기준 전 세계 평균 약 10일까지 단축됐지만, 여전히 일부 조직은 수개월입니다. 짧을수록 좋습니다.

### Incident Response Playbook

알림이 뜨면 무엇을 할 것인가? 이게 미리 정해져 있어야 합니다. 한밤중에 누군가 깨어나서 즉흥적으로 결정하지 않도록 해야 합니다.
...
Timeout이 없으면 한 의존 서비스의 느림이 우리 스레드 풀을 모두 점유합니다. 서비스가 다운됩니다. **이게 보안 문제인 이유**: 의도적인 slow request가 DoS로 이어지기 때문입니다.
...
같은 멱등키로 두 번 호출해도 결제는 한 번만 일어납니다. 네트워크 timeout으로 클라이언트가 재시도해도 안전합니다.
...
자원을 분리해서 한쪽 장애가 다른 쪽으로 번지지 않게 합니다. 선박의 격벽에서 따온 이름입니다.
...
DB connection pool, HTTP client pool도 같은 원리로 분리합니다.
...
평상시에 일부러 장애를 일으켜서 시스템이 어떻게 반응하는지 관찰하는 연습입니다. Netflix가 시작한 "Chaos Monkey"가 유명합니다.
...
OAuth 프로바이더 응답 timeout. 코드가 fail open으로 짜여 있어 timeout = "익명 사용자로 진행". 일부 엔드포인트에서 권한 체크 없이 통과합니다. 이 시간 동안 데이터가 노출됩니다.

**패턴 2: 알림 누락 → 며칠간 침투 미발견 → 데이터 대량 유출**

공격자가 SQL Injection으로 진입합니다. 비정상 패턴이 로그에 남았지만 알림 룰이 없어서 안 잡힙니다. 4일 동안 천천히 데이터를 export합니다. 발견은 외부(고객 신고)에서 이루어집니다. Equifax 패턴입니다.

**패턴 3: Circuit Breaker 부재 → 의존 서비스 느림 → DoS**

PG사 응답 시간이 평소 100ms에서 5초로 증가합니다. timeout이 30초로 설정돼 있습니다. 우리 서비스의 모든 스레드가 결제 호출 대기에 묶입니다. 다른 요청은 처리할 수 없습니다. 사실상 다운됩니다. 보안 통제(rate limit, WAF)도 멈춥니다.

**패턴 4: 디버그 코드 잔존 → 백도어**

개발 단계의 `if (req.headers['x-skip-auth'] === 'true') return next()` 가 그대로 production. 누군가 우연히 또는 의도적으로 발견합니다. 인증 우회가 가능합니다. CI/CD 단계의 grep 검사가 있었다면 잡혔을 사고입니다.
