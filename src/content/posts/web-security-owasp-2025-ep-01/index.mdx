...
date: 2026-04-25
tags: ["series", "develop", "backend", "security", "owasp", "authentication", "authorization", "jwt", "mfa", "passkey", "idor"]
thumbnail: ./cover.svg
draft: false
---
...
![인증과 인가의 차이를 좌우 두 열로 정리한 다이어그램. 왼쪽 파란 영역은 AUTHENTICATION 인증으로 "너 누구야?"라는 질문을 던지며 사용자의 신원과 자격증명 진위를 확인하고, 비밀번호·OTP·Passkey·OAuth·SAML·SSO를 대표 수단으로 든다. 오른쪽 녹색 영역은 AUTHORIZATION 인가로 "너 이거 해도 돼?"라는 질문을 던지며 리소스 접근 권한과 수행 가능 액션 범위를 확인하고, 역할 기반(RBAC)·소유자 검증·정책 기반(ABAC)을 대표 수단으로 든다. 하단에는 요청 들어옴 → 인증으로 식별 → 인가로 확인 → 응답 반환의 네 단계 흐름 박스가 화살표로 이어져, 두 단계가 순서대로 적용되어야 함을 보여준다.](sec-01-01-authn-vs-authz.svg)
...
OWASP Top 10에서 **2021년부터 5년 내내 1위**를 지키고 있는 카테고리입니다. 2025년판에서는 2021년의 A10 SSRF(Server-Side Request Forgery)까지 흡수해서 더 넓어졌습니다. 테스트된 애플리케이션의 **평균 3.73%에 이 카테고리의 취약점이 있다**는 것이 OWASP의 집계입니다.
...
이 두 사건의 공통점은 **단순한 ID 변조**였다는 점입니다. URL의 숫자를 바꾸거나 API 파라미터를 건드리는 수준이었습니다.
...
![IDOR 공격 시나리오를 취약한 구현(BEFORE)과 안전한 구현(AFTER)으로 좌우 비교한 다이어그램. 공통 시나리오는 User B의 토큰으로 User A의 주문 ID 1042에 GET /api/orders/1042를 호출하는 상황. 왼쪽 취약한 구현은 1단계 토큰 검증 통과 → 2단계 SELECT * FROM orders WHERE id = 1042 쿼리(소유자 검증 누락) → 3단계 User A의 주문이 User B에게 노출되어 HTTP 200 OK가 떨어지고 IDOR 취약점 발생으로 표시된다. 오른쪽 안전한 구현은 같은 토큰 검증 후 SELECT * FROM orders WHERE id = 1042 AND user_id = :current_user 쿼리로 소유자 제약이 함께 들어가, User B는 User A 주문을 볼 수 없어 HTTP 403/404 접근 거부가 반환된다. 양쪽 모두 근본 원인과 핵심 원칙이 하단에 적혀 있다.](sec-01-02-idor-flow.svg)
...
인접 ID도 같이 테스트합니다. `1041`, `1043` 등입니다. UUID라도 **권한 체크가 없으면 여전히 취약**합니다. UUID는 추측 난이도만 올릴 뿐이지, 유출되면 바로 뚫립니다.
...
![세션 기반과 토큰 기반 인증을 좌우 두 열로 비교한 다이어그램. 왼쪽 파란 열은 SESSION BASED(Stateful)로 클라이언트→서버→세션 저장소 세 노드가 일렬로 있고, 로그인 시 서버가 저장소에 세션을 저장한 뒤 클라이언트에는 세션 ID만 쿠키로 전달한다. 장점은 즉시 무효화 가능·세션 정보 변경 자유·쿠키 크기 작음, 단점은 서버 상태 관리 필요·수평 확장 시 공유 설계·CSRF 대비. 대표 사용처는 전통적 서버 렌더링 웹과 관리자 페이지. 오른쪽 보라 열은 TOKEN BASED JWT(Stateless)로 클라이언트와 서버만 있고 저장소 자리는 점선으로 "없음"이라 표시된다. 토큰 자체에 사용자 정보와 서명이 포함되어 매 요청마다 서명만 검증한다. 장점은 서버 상태 불필요·수평 확장 용이·MSA에 적합, 단점은 즉시 무효화 어려움·토큰 크기 큼·payload 노출 가능. 대표 사용처는 SPA·모바일 앱·마이크로서비스 API.](sec-01-04-session-vs-token.svg)
...
![JWT 구조와 alg:none 공격을 보여주는 다이어그램. 상단에는 일반적인 JWT가 점(.)으로 분리된 세 부분(HEADER·PAYLOAD·SIGNATURE)으로 그려지고, 각 부분의 base64 인코딩 문자열 아래에 디코드된 내용이 펼쳐진다. HEADER는 \{"alg":"HS256","typ":"JWT"\}, PAYLOAD는 \{"sub":"user123","role":"user"\}, SIGNATURE는 HMACSHA256(header + "." + payload, secret)로 표시된다. 강조 메모로 "Header와 Payload는 단순 base64 인코딩이라 누구나 디코드 가능하며 위조 방지는 서명 검증 하나에 전적으로 의존한다"고 적혀 있다. 점선 구분 아래는 alg:none 공격 시나리오로, header를 \{"alg":"none"\}으로 바꾸고 payload의 role을 "admin"으로 변조한 뒤 signature를 빈 문자열로 둔 위조 토큰이 호박색으로 표시된다. 취약한 라이브러리는 서명 검증을 건너뛰어 관리자 권한이 부여된다.](sec-01-03-jwt-structure.svg)
...
![인증 강도 스펙트럼을 4단계로 보여주는 다이어그램. 왼쪽 세로 막대는 WEAK(빨강)에서 STRONG(녹색)으로 색상이 그라데이션되고, 오른쪽에는 4개의 카드가 위에서 아래로 쌓인다. 1단계 비밀번호만은 적색, 2단계 비밀번호+SMS OTP는 호박색, 3단계 비밀번호+TOTP 앱(Google Authenticator 등)은 파란색, 4단계 Passkey·FIDO2·WebAuthn은 녹색. 각 카드 오른쪽에는 Credential Stuffing·Phishing·SIM Swap 세 공격에 대한 저항성을 ✓(녹색)·✗(빨강)·△(호박)으로 표시한 동그라미가 있다. 1단계는 세 공격 모두 ✗ 또는 무관, 2단계는 Credential Stuffing만 막고 Phishing·SIM Swap에 ✗, 3단계는 SIM Swap을 막고 Phishing은 △로 부분 취약, 4단계는 세 공격 모두 ✓로 가장 강하다. 위로 갈수록 구현은 쉽지만 공격에 취약하고, 아래로 갈수록 피싱·탈취 저항성이 높다는 캡션이 붙어 있다.](sec-01-05-auth-strength-spectrum.svg)
...
직접 구현하지 마세요. Passport.js, Spring Security, Django auth, Rails Devise, Next-Auth 등 잘 검증된 라이브러리가 있습니다. 직접 짠 대부분의 JWT 검증 코드에는 취약점이 있습니다.
...
**OAuth 2.1 · PKCE 필수화.** SPA와 모바일 앱에서 OAuth를 쓴다면 PKCE(Proof Key for Code Exchange)가 기본이어야 합니다. Authorization Code 가로채기 공격을 방어합니다.
...
이런 연쇄를 끊는 핵심은 **Defense in Depth**입니다. 한 레이어가 뚫려도 다음 레이어가 막도록 설계하는 것입니다. 인증이 뚫려도 인가가 버티고, 인가가 뚫려도 쿼리 제약이 버티고, 거기도 뚫려도 WAF가 버티는 식입니다.
