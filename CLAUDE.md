...
- [ThreeColumnLayout](src/layouts/ThreeColumnLayout.astro) — sticky 3-column on desktop, fixed slide-in panels on mobile. **Single global toggle = `body.panel-active`**, cleared only when both panels are closed. It unifies three input sources: hover-reveal (desktop), explicit toggle buttons, and a 50px-threshold horizontal swipe (mobile). Swipe is suppressed if the touch target has a horizontally-scrollable ancestor (`hasHorizontalScrollableAncestor`) — this prevents code-block left/right drags from yanking the panel.
  ...
  A swipe that *opens* a panel additionally has to happen at the horizontal edge of the document. `horizontalPanRoom()` measures how much room is left to pan left/right — from `visualViewport.pageLeft`/`width` when available, else `scrollX`/`clientWidth`, against `documentElement.scrollWidth` — and `atPanEdge()` requires that room to be ≤ `EDGE_TOLERANCE` (2px) at **both** `touchstart` and `touchend`. So pinch-zoomed panning no longer opens panels: you pan to the edge, lift, and swipe again. Pinch-zoom panning is *not* a document scroll — it only shows up as a visual-viewport offset, which is why `hasHorizontalScrollableAncestor` never caught it. Swipes that *close* an open panel skip the gate, so a panel is always dismissable while zoomed. Unzoomed with no document-level horizontal scroll, both rooms are 0 and the gate is a no-op — the plain swipe behaves as before.
...
## Proof drawer (dev only)

[src/components/dev/ProofDrawer.astro](src/components/dev/ProofDrawer.astro) is a proofreading aid that [PostLayout](src/layouts/PostLayout.astro) renders **only when `import.meta.env.DEV` is true** — `astro dev` shows it, `astro build` / `astro preview` ship nothing (no markup, script, or style). Select text inside `.post-content` → a floating "코멘트" button appears → the note lands in the drawer (bottom-right card on desktop, bottom sheet under 600px).

- **Storage**: `localStorage["pielog:proof:v1"]` = `{ [slug]: ProofComment[] }`. Each comment expires **7 days after creation**; pruning happens in `loadStore()` ([src/lib/proofComments.ts](src/lib/proofComments.ts)) and is written back immediately so other tabs don't resurrect expired items.
- **Slug** is derived from `location.pathname` exactly like the views counter — renaming a post directory orphans its notes.
- **Copy**: "전체 복사" (this post) and "모든 글 복사" (every post, inside the "다른 글" section) put a Markdown block (slug, `src/content/posts/<slug>/` path, numbered `[heading]` → `> quote` → `→ note` entries) on the clipboard for pasting into an AI session. If `navigator.clipboard` is unavailable (non-HTTPS LAN access), a `<dialog>` with the text pre-selected opens instead.
- **Selection capture** caches the last non-empty selection and clears it only after a short grace period once the selection collapses — on iOS the selection collapses before `click` fires. While a selection is alive the FAB turns into "선택 영역에 코멘트" and the panel's "＋ 코멘트" becomes "＋ 선택 영역에 코멘트"; both open the composer with the cached quote. The composer is a standalone fixed card (bottom-right / bottom sheet) holding only the textarea and buttons, and **closes the drawer when it opens** so the body stays readable. The quoted range is not shown in the card — it is marked in the body via the CSS Custom Highlight API (`CSS.highlights`, name `proof-quote`, color `--proof-mark` on `:root`); browsers without it get the quote's first 40 chars in the placeholder instead. Opening the composer while it is already open retargets the quote and keeps the typed text. Cancel/Esc asks for confirmation when the textarea is non-empty; saving closes the card and shows a toast. The floating button next to the selection is shown only on `(hover: hover) and (pointer: fine)` devices, because on touch devices the system selection callout covers it. "＋ 코멘트" without a cached selection creates a quote-less whole-post comment. The nearest `h2–h4` heading is still stored and emitted in the copy text as `[## …]`, but not shown in the UI.
- [ThreeColumnLayout](src/layouts/ThreeColumnLayout.astro) ignores a touch that ends with a live text selection (and a `pointerenter` with a mouse button held) so dragging selection handles doesn't open the side panels. This is production behavior, not dev-only.
- The drawer stops `touchstart`/`touchend` propagation so ThreeColumnLayout's swipe detection ignores it, and sets `pointer-events: auto` on its children because `body.panel-active .main-content` is otherwise inert.
- **Why `is:inline`**: Astro collects processed `<script>`/`<style>` by *import graph*, not by render (`plugin-analyzer.js` adds every `?astro&type=script` it sees; CSS attaches to pages via `walkParentInfos`, which follows dynamic importers too). A processed variant would ship an orphan JS chunk and dead `.proof-*` CSS in production even though the component never renders. So the drawer's script is an inline `<script type="module">` that imports `/src/lib/proofComments.ts` straight from the dev server, and its CSS is `<style is:inline>` using the `--color-*` custom properties directly — neither enters the build graph.
- **Mobile keyboard / scroll**: iOS doesn't shrink the layout viewport for the keyboard, so the composer's `bottom` adds `--proof-keyboard-inset`, computed from `visualViewport` on every resize/scroll. Opening/closing the composer schedules one smooth scroll that brings the marked range into the visible area (minus the card): immediately on fine-pointer devices, and on touch devices only after `visualViewport` has stopped resizing for 250ms (700ms fallback if no resize arrives) — i.e. after the keyboard finished animating, because iOS scrolls the page itself while the keyboard opens and never scrolls back. On touch devices the card is rendered transparent (`is-preparing`) while this happens — the textarea is focused so the keyboard rises, and the card fades in only after the viewport settled and the scroll adjustment finished — so the keyboard-tracking movement of its `bottom` (which also transitions) is never visible. Textarea focus/blur change nothing: the card and the mark stay until the card is closed, and closing also collapses the native selection. Show/hide animations keep the `hidden` toggle and rely on `@starting-style` + `transition-behavior: allow-discrete`; older browsers just switch instantly.
- `proofComments.ts` has no DOM dependency; sanity-check it with `node --experimental-strip-types` if you change the storage shape or copy format.
